...
def move_test():
    arm_joint_positions_L = [0, -0.9, 0, 0.9, 0.0, 1.57, 0.0]
    arm_joint_positions_R = [0, -0.1, 0, 0.9, 0.0, 1.57, 0.0]
    move_fetch_obj = MoveFetch()

    side = "LEFT"
    msg = Twist()
    pub = rospy.Publisher("/base_controller/command", Twist, queue_size=1000)
    ...
    while not rospy.is_shutdown():
        if forward:
            if time.time()-curr>3:
                forward = False
                curr =time.time()    
            msg.linear.x = 1 #random.uniform(0, 1)
        else:
            if time.time()-curr>3:
                forward=True
                curr = time.time()
            msg.linear.x = -0.1#random.uniform(-1, 0)
