...
def friction_base_tests():
    ...
    arm_pose_positions_L.position.x = 0.0
    arm_pose_positions_L.position.y = -0.65
    arm_pose_positions_L.position.z= 0.9
    arm_pose_positions_L.orientation.x= 0.5
    arm_pose_positions_L.orientation.y= 0.5
    arm_pose_positions_L.orientation.z= -0.5
    arm_pose_positions_L.orientation.w= 0.5

    arm_pose_positions_R.position.x= 0.0
    arm_pose_positions_R.position.y= 0.65
    arm_pose_positions_R.position.z= 0.9
    arm_pose_positions_R.orientation.x= -0.5
    arm_pose_positions_R.orientation.y= 0.5
    arm_pose_positions_R.orientation.z= 0.5
    arm_pose_positions_R.orientation.w= 0.5
    ...
    while not rospy.is_shutdown():
        if side == "LEFT":
            side = "RIGHT"
            move_fetch_obj.move_manager(pose_requested=arm_pose_positions_L,
                                        joints_array_requested=None,
                                        movement_type_requested="TCP")
        else:
            side = "LEFT"
            move_fetch_obj.move_manager(pose_requested=arm_pose_positions_R,
                                        joints_array_requested=None,
                                        movement_type_requested="TCP")
        rate_obj.sleep()
        ps = move_fetch_obj.group.get_current_pose().pose
        rospy.loginfo(side)
        rospy.loginfo(ps.position.x)
        rospy.loginfo(ps.position.y)
        rospy.loginfo(ps.position.z)
        rospy.loginfo(ps.orientation.x)
        rospy.loginfo(ps.orientation.y)
        rospy.loginfo(ps.orientation.z)
        rospy.loginfo(ps.orientation.w)
...
def updown_test():
    ...
    arm_pose_positions_R.position.x= 0.7
    arm_pose_positions_R.position.y= 0.0
    arm_pose_positions_R.position.z= 0.3
    arm_pose_positions_R.orientation.x= -0.5
    arm_pose_positions_R.orientation.y= 0.5
    arm_pose_positions_R.orientation.z= 0.5
    arm_pose_positions_R.orientation.w= 0.5
    ...
    while not rospy.is_shutdown():
        ...
        rospy.loginfo(move_fetch_obj.group.get_end_effector_link())
        ps = move_fetch_obj.group.get_current_pose().pose
        rospy.loginfo(ps.position.x)
        rospy.loginfo(ps.position.y)
        rospy.loginfo(ps.position.z)
